...
// Настройки
const bool DEBUG_OUTPUT = false;// true; //false; // Флаг вывода отладочной информации в Serial
...
void setup() {
    ...
    server.on("/redirect", []() {
        ...
        server.send(302, "text/plain", "");  // Отправляет HTTP-ответ с кодом 302 (временное перенаправление).
    });
